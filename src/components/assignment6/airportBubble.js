...
function AirportBubble(props){
    ...
    if(selectedAirline){
        ...
        // Sort cities by count in ascending order to ensure large circles are in the back
        cities.sort((a, b) => a.Count - b.Count);

        let simulation = forceSimulation(cities)
        .force('x', forceX(width / 2).strength(0.02))
        .force('y', forceY(height / 2).strength(0.02))
        .force('collide', forceCollide(d => raidus(d.Count)))
        .velocityDecay(0.2)
        .stop();
        for(let i = 0; i < 200; i++) { simulation.tick(); }


        // Update cities with simulation results for rendering
        ...
        return <g>
              {cities.map((city, idx) => {
                    let fillColor = idx >= cities.length - 5 ? "#ADD8E6" : "#2a5599";
                    return (
                        <g key={idx}>
                            <circle
                                cx={city.x}
